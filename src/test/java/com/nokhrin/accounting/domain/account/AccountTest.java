...
class AccountTest {
    ...
    @Test
    void create_validParams_activeAccount(){
        Account account=Account.create(new BigDecimal("100"), HOLDER);
        assertAll(
    ...
    void create_negativeBalance_throwsException(){
        assertThrows(IllegalArgumentException.class,
                ()->Account.create(new BigDecimal("-1"), HOLDER));
    }
    ...
    void block_blockedAccount_throwsException(){
        Account accountBlocked=Account.create(BigDecimal.ZERO, HOLDER).block();
        assertThrows(IllegalStateException.class, accountBlocked::block);
    }

    @Test
    void activate_blockedAccount_transitionsToActive(){
        Account blocked=Account.create(BigDecimal.ZERO, HOLDER).block();
